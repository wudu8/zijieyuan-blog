...
person.name = 'ZiJieYuan';
console.log(person.name) // ZiJieYuan
```
...
![构造函数和实例原型的关系图](./imgs/prototype1.png)
...
![实例与实例原型的关系图](./imgs/prototype2.png)
...
![实例原型与构造函数的关系图](./imgs/prototype3.png)
...
person.name = 'WuDu';
console.log(person.name) // WuDu

delete person.name;
console.log(person.name) // ZiJieYuan
```

在这个例子中，我们给实例对象 person 添加了 name 属性，当我们打印 person.name 的时候，结果自然为 WuDu。

但是当我们删除了 person 的 name 属性时，读取 person.name，从 person 对象中找不到 name 属性就会从 person 的原型也就是 person.\_\_proto\_\_ ，也就是 Person.prototype中查找，幸运的是我们找到了  name 属性，结果为 ZiJieYuan。
